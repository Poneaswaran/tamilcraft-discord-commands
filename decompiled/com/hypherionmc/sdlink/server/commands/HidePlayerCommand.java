...
public final class HidePlayerCommand {
    public static void register(CraterRegisterCommandEvent event) {
        CraterCommand command = CraterCommand.literal((String)"hideplayer").requiresPermission(4).withNode("sdlink.muteplayer").withGameProfilesArgument("username", (player, profiles, ctx) -> {
            if (profiles.isEmpty()) {
                ctx.sendSuccess(() -> Text.literal((String)SDText.translate("error.hiding.no_account_supplied").toString()), true);
                return 1;
            }
