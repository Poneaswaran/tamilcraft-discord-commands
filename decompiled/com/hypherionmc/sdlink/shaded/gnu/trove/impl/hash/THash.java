...
import com.hypherionmc.sdlink.shaded.gnu.trove.impl.PrimeFinder;
...
implements Externalizable {
    ...
    public THash(int initialCapacity, float loadFactor) {
        ...
        this.setUp(THash.saturatedCast(THash.fastCeil((double)initialCapacity / (double)loadFactor)));
    }

    protected static long fastCeil(double v) {
        long possible_result = (long)v;
        if (v - (double)possible_result > 0.0) {
            ++possible_result;
        }
    ...
    public void ensureCapacity(int desiredCapacity) {
        if (desiredCapacity > this._maxSize - this.size()) {
            this.rehash(PrimeFinder.nextPrime(Math.max(this._size + 1, THash.saturatedCast(THash.fastCeil((double)(desiredCapacity + this._size) / (double)this._loadFactor) + 1L))));
    ...
    public void compact() {
        this.rehash(PrimeFinder.nextPrime(Math.max(this._size + 1, THash.saturatedCast(THash.fastCeil((double)this._size / (double)this._loadFactor) + 1L))));
        this.computeMaxSize(this.capacity());
    ...
    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        if (old_factor != this._loadFactor) {
            this.setUp(THash.saturatedCast((long)Math.ceil(10.0 / (double)this._loadFactor)));
        }
    }
}
