...
import com.hypherionmc.sdlink.shaded.fasterxml.jackson.databind.deser.std.FactoryBasedEnumDeserializer;
...
import com.hypherionmc.sdlink.shaded.fasterxml.jackson.databind.util.CompactStringObjectMap;
...
implements ContextualDeserializer {
    ...
    public static JsonDeserializer<?> deserializerForCreator(DeserializationConfig config, Class<?> enumClass, AnnotatedMethod factory2, ValueInstantiator valueInstantiator, SettableBeanProperty[] creatorProps) {
        if (config.canOverrideAccessModifiers()) {
            ClassUtil.checkAndFixAccess(factory2.getMember(), config.isEnabled(MapperFeature.OVERRIDE_PUBLIC_ACCESS_MODIFIERS));
        }
