...
import com.hypherionmc.sdlink.shaded.fasterxml.jackson.core.io.doubleparser.FastDoubleMath;
...
class FftMultiplier {
    ...
    private static ComplexVector calculateRootsOfUnity(int n) {
        ...
        ComplexVector roots2 = new ComplexVector(n);
        roots2.set(0, 1.0, 0.0);
        double cos = COS_0_25;
        double sin = SIN_0_25;
        roots2.set(n / 2, cos, sin);
        double angleTerm = 1.5707963267948966 / (double)n;
        for (int i = 1; i < n / 2; ++i) {
            double angle = angleTerm * (double)i;
            cos = Math.cos(angle);
            sin = Math.sin(angle);
            roots2.set(i, cos, sin);
            roots2.set(n - i, sin, cos);
        }
        return roots2;
    }

    private static void fft(ComplexVector a, ComplexVector[] roots2) {
        int s;
        int n = a.length;
        ...
        for (s = logN; s >= 2; s -= 2) {
            ComplexVector rootsS = roots2[s - 2];
            int m = 1 << s;
            for (int i = 0; i < n; i += m) {
                for (int j = 0; j < m / 4; ++j) {
                    omega1.set(rootsS, j);
    ...
    private static ComplexVector[] getRootsOfUnity2(int logN) {
        ComplexVector[] roots2 = new ComplexVector[logN + 1];
        for (int i = logN; i >= 0; i -= 2) {
            if (i < 20) {
                if (ROOTS2_CACHE[i] == null) {
                    FftMultiplier.ROOTS2_CACHE[i] = FftMultiplier.calculateRootsOfUnity(1 << i);
                }
                roots2[i] = ROOTS2_CACHE[i];
                continue;
            }
            roots2[i] = FftMultiplier.calculateRootsOfUnity(1 << i);
        }
        return roots2;
    }
    ...
    private static void ifft(ComplexVector a, ComplexVector[] roots2) {
        int n = a.length;
        int logN = 31 - Integer.numberOfLeadingZeros(n);
        MutableComplex a0 = new MutableComplex();
        MutableComplex a1 = new MutableComplex();
        ...
        MutableComplex omega2 = new MutableComplex();
        while (s <= logN) {
            ComplexVector rootsS = roots2[s - 1];
            int m = 1 << s + 1;
            for (int i = 0; i < n; i += m) {
                for (int j = 0; j < m / 4; ++j) {
                    omega1.set(rootsS, j);
    ...
    static BigInteger multiplyFft(BigInteger a, BigInteger b) {
        ...
        ComplexVector[] roots2 = FftMultiplier.getRootsOfUnity2(logFFTLen);
        ComplexVector aVec = FftMultiplier.toFftVector(aMag, fftLen2, bitsPerPoint);
        aVec.applyWeights(roots2[logFFTLen]);
        FftMultiplier.fft(aVec, roots2);
        ComplexVector bVec = FftMultiplier.toFftVector(bMag, fftLen2, bitsPerPoint);
        bVec.applyWeights(roots2[logFFTLen]);
        FftMultiplier.fft(bVec, roots2);
        aVec.multiplyPointwise(bVec);
        FftMultiplier.ifft(aVec, roots2);
        aVec.applyInverseWeights(roots2[logFFTLen]);
        return FftMultiplier.fromFftVector(aVec, signum, bitsPerPoint);
    }
    ...
    static BigInteger squareFft(BigInteger a) {
        ...
        ComplexVector vec = FftMultiplier.toFftVector(mag, fftLen, bitsPerPoint);
        ComplexVector[] roots2 = FftMultiplier.getRootsOfUnity2(logFFTLen);
        vec.applyWeights(roots2[logFFTLen]);
        FftMultiplier.fft(vec, roots2);
        vec.squarePointwise();
        FftMultiplier.ifft(vec, roots2);
        vec.applyInverseWeights(roots2[logFFTLen]);
        return FftMultiplier.fromFftVector(vec, 1, bitsPerPoint);
    }
