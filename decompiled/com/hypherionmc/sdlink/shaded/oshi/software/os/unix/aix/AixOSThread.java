...
extends AbstractOSThread {
    ...
    public boolean updateAttributes() {
        ...
        if (threadListInfoPs.size() > 2) {
            ...
            for (String threadInfo : threadListInfoPs) {
                Map<AixOSProcess.PsThreadColumns, String> threadMap = ParseUtil.stringToEnumMap(AixOSProcess.PsThreadColumns.class, threadInfo.trim(), ' ');
    ...
    private boolean updateAttributes(Map<AixOSProcess.PsThreadColumns, String> threadMap) {
        this.threadId = ParseUtil.parseIntOrDefault(threadMap.get((Object)AixOSProcess.PsThreadColumns.TID), 0);
        this.state = AixOSProcess.getStateFromOutput(threadMap.get((Object)AixOSProcess.PsThreadColumns.ST).charAt(0));
        this.priority = ParseUtil.parseIntOrDefault(threadMap.get((Object)AixOSProcess.PsThreadColumns.PRI), 0);
        return true;
    }
}
