 ...
 */
package com.hypherionmc.sdlink.shaded.oshi.util.platform.linux;
...
public final class ProcPath {
    ...
    private static String queryProcConfig() {
        String procPath = GlobalConfig.get("com.hypherionmc.sdlink.shaded.oshi.util.proc.path", "/proc");
        procPath = '/' + procPath.replaceAll("/$|^/", "");
        if (!new File(procPath).exists()) {
            throw new GlobalConfig.PropertyException("com.hypherionmc.sdlink.shaded.oshi.util.proc.path", "The path does not exist");
        }
        return procPath;
    }
}
