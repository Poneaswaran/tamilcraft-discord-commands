...
public class WmiQueryHandler {
    ...
    private static int globalTimeout = GlobalConfig.get("com.hypherionmc.sdlink.shaded.oshi.util.wmi.timeout", -1);
    ...
    protected void handleComException(WbemcliUtil.WmiQuery<?> query, COMException ex) {
        LOG.warn("COM exception querying {}, which might not be on your system. Will not attempt to query it again. Error was {}: {}", new Object[]{query.getWmiClassName(), ex.getHresult().intValue(), ex.getMessage()});
    }
    ...
    static {
        if (globalTimeout == 0 || globalTimeout < -1) {
            throw new GlobalConfig.PropertyException("com.hypherionmc.sdlink.shaded.oshi.util.wmi.timeout");
        }
