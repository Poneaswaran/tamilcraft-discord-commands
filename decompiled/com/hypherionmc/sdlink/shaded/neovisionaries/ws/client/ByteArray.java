...
class ByteArray {
    ...
    public void put(byte[] source2) {
        if (this.mBuffer.capacity() < this.mLength + source2.length) {
            this.expandBuffer(this.mLength + source2.length + 1024);
        }
        this.mBuffer.put(source2);
        this.mLength += source2.length;
    }

    public void put(byte[] source2, int index, int length) {
        if (this.mBuffer.capacity() < this.mLength + length) {
            this.expandBuffer(this.mLength + length + 1024);
        }
        this.mBuffer.put(source2, index, length);
        this.mLength += length;
    }

    public void put(ByteArray source2, int index, int length) {
        this.put(source2.mBuffer.array(), index, length);
    }
    ...
    public void setBit(int bitIndex, boolean bit) {
        ...
        value = bit ? (value |= 1 << shift) : (value &= ~(1 << shift));
