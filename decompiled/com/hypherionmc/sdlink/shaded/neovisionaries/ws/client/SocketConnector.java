...
import com.hypherionmc.sdlink.shaded.neovisionaries.ws.client.HostnameUnverifiedException;
import com.hypherionmc.sdlink.shaded.neovisionaries.ws.client.OkHostnameVerifier;
import com.hypherionmc.sdlink.shaded.neovisionaries.ws.client.ProxyHandshaker;
import com.hypherionmc.sdlink.shaded.neovisionaries.ws.client.SocketInitiator;
...
class SocketConnector {
    ...
    SocketConnector(SocketFactory socketFactory, Address address, int timeout2, String[] serverNames, int socketTimeout) {
        this(socketFactory, address, timeout2, socketTimeout, serverNames, null, null, null, 0);
    }

    SocketConnector(SocketFactory socketFactory, Address address, int timeout2, int socketTimeout, String[] serverNames, ProxyHandshaker handshaker, SSLSocketFactory sslSocketFactory, String host, int port) {
    ...
    private void connectSocket() throws WebSocketException {
        ...
        try {
            ...
            String message = String.format("Failed to connect to %s'%s': %s", proxied ? "the proxy " : "", this.mAddress, e.getMessage());
    ...
    private void setSoTimeout(int timeout2) throws WebSocketException {
        assert (this.mSocket != null);
        try {
            this.mSocket.setSoTimeout(timeout2);
        }
        catch (SocketException e) {
            String message = String.format("Failed to set SO_TIMEOUT: %s", e.getMessage());
            throw new WebSocketException(WebSocketError.SOCKET_CONNECT_ERROR, message, e);
        }
    }
    ...
    private void handshake() throws WebSocketException {
        ...
        try {
            ...
            String message = String.format("Handshake with the proxy server (%s) failed: %s", this.mAddress, e.getMessage());
        ...
        try {
            ...
            String message = String.format("SSL handshake with the WebSocket endpoint (%s) failed: %s", this.mAddress, e.getMessage());
