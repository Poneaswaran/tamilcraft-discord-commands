...
implements SortedSnowflakeCacheView<T> {
    ...
    public Iterator<T> iterator() {
        try (UnlockHook hook = this.readLock();){
            ISnowflake[] arr = this.elements.values((ISnowflake[])this.emptyArray);
            Arrays.sort(arr, this.comparator);
            ObjectArrayIterator<ISnowflake> objectArrayIterator = new ObjectArrayIterator<ISnowflake>(arr);
