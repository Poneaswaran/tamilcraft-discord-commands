...
implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            boolean bl = hasLock = this.queueLock.tryLock() || this.queueLock.tryLock(10L, TimeUnit.SECONDS);
            if (!hasLock) {
                this.scheduleNext();
                return;
            }
    ...
    private void scheduleNext() {
        try {
            ...
            if (this.api.getStatus() == JDA.Status.SHUTTING_DOWN || this.api.getStatus() == JDA.Status.SHUTDOWN) {
                LOG.debug("Rejected task after shutdown", (Throwable)ex);
            }
            LOG.error("Was unable to schedule next packet due to rejected execution by threadpool", (Throwable)ex);
        }
    }
