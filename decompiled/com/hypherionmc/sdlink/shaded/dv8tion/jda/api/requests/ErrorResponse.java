...
public enum ErrorResponse {
    ...
    private ErrorResponse(int code, String meaning) {
        this.code = code;
    ...
    @Nonnull
    public static Predicate<Throwable> test(ErrorResponse ... responses) {
        Checks.noneNull((Object[])responses, "ErrorResponse");
        EnumSet<ErrorResponse> set = EnumSet.noneOf(ErrorResponse.class);
        Collections.addAll(set, responses);
        return ErrorResponse.test(set);
    }
    ...
    private static Predicate<Throwable> test(@Nonnull EnumSet<ErrorResponse> responses) {
        return error -> error instanceof ErrorResponseException && responses.contains((Object)((ErrorResponseException)error).getErrorResponse());
    }

    @Nonnull
    public static ErrorResponse fromCode(int code) {
        for (ErrorResponse error : ErrorResponse.values()) {
            if (code != error.getCode()) continue;
            return error;
        }
        return SERVER_ERROR;
    }
